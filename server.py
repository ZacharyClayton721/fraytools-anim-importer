...
from flask import Flask, render_template, jsonify, request, Response, session, current_app, send_file
    ...
    def recurse(node):
        ...
        for name, content in list(node.items()):
            ...
            if name.endswith(".png"):
                ...
                node[asset_path] = {'_type':'json',
                                    '_content':sprite_metadata}
...
def parseFFE(text):
    ...
    for block in blocks:
        ...
        if group and image and xaxis and yaxis:
            ...
            entries[sprite_name] = {"file_name":file_name_val,
                                  "xaxis": xaxis_val,
                                  "yaxis": yaxis_val}
...
def generateGIF():
    ...
    ce_data = session['folder_tree'][project_folder]['library']['entities'][data['entityName']]
                ...
                if al == layer_data['$id'] and layer_data['type'] == 'IMAGE':
                    ...
                    for lk in layer_keyframes:
                        ...
                        for x in range(0,keyframe_data['length']):
                            ...
                            master_image.append([sprite_guids[symbol_data['imageAsset']]['data'],
                                                 symbol_data['alpha'],
                    ...
                    for img_info in master_image:
                        ...
                        draw.line([(canvas_center_x, canvas_center_y - plus_sign_size), 
                                   (canvas_center_x, canvas_center_y + plus_sign_size)], fill="gray", width=1)
                        
                        # Horizontal line of the plus sign at the canvas center
                        draw.line([(canvas_center_x - plus_sign_size, canvas_center_y), 
                                   (canvas_center_x + plus_sign_size, canvas_center_y)], fill="gray", width=1)
                    ...
                    duration=1000 // 60,  # Frame duration in milliseconds (1000 ms / 60 FPS = ~16.67 ms)
                    loop=0,
                    transparency=0,
...
def generateAirAnimNames(air_data):
    ...
    while i < len(lines):
        ...
        if not line or line.startswith(';'):
            if line.startswith(';') and '[Begin Action' in (lines[i+1] if i+1 < len(lines) else ''):
        ...
        if re.match(r'^\d+,\d+,\s*-?\d+,\s*-?\d+,\s*-?\d+', line):
            ...
            frame = {
                ...
                'offset': offset,
                'duration': duration,
                'hurtboxes': pending_clsn2.popleft() if pending_clsn2 else default_clsn2,
            ...
            actions[current_animation or f"Action_{current_action}"]['frames'].append(frame)
...
def generateAnimPreview():
    ...
    def get_canvas_bounds(frames, sprite_names):
        ...
        max_x, max_y = 0, 0
        for frame in frames:
            img = Image.open(io.BytesIO(sprite_names[session['ffe_data'][frame['image']]['file_name']])).convert("RGBA")
            ox, oy = frame['offset']
            min_x = min(min_x, ox)
            min_y = min(min_y, oy)
...
def add_folder_to_zip(zip_file, folder_tree, current_path=""):
    for name, content in folder_tree.items():
        ...
        if isinstance(content, dict) and '_type' in content and content['_type'] == 'json':
...
def generateAnimation(layers,name):
    ...
    animation_data = {'$id':animation_guid,
                      'layers':layers,
                      'name':name,
                      'pluginMetadata':{}}
...
def generateLayer(keyframes, item_type, hitbox_index=0, hurtbox_index=0):
    ...
    if item_type not in layer_configs:
        raise ValueError(f"Unknown item_type '{item_type}'. Expected one of: {list(layer_configs.keys())}")
        
    layer_data = {**base_layer, **layer_configs[item_type]}
    return layer_guid, layer_data
            
def generateSymbol(image_asset=None, item_type='IMAGE', x=0,y=0,scaleX=1,scaleY=1,angle=0):
...
def generateKeyframe(img_guid, item_type, duration):
    ...
    keyframe_data = {'$id':keyframe_guid,
                     'length':duration,
                     'pluginMetadata':{},
                     'symbol':img_guid,
                     'tweenType':'LINEAR',
    ...
    for ad in air_data:
            ...
            for i in range(len(index_list)):
                if i < len(boxes):
                    ...
                    box_guid, symbol_data = generateSymbol(
                        ...
                        x=hd[2], y=hd[3],
                        scaleX=-hd[2]+hd[4], scaleY=-hd[3]+hd[5]
                    )
        ...
        for f in frames:
            ...
            try:
                ...
                img_meta = session['sprite_data'][session['sprite_name']][ffe_data['file_name']+'.meta']['_content']
            ...
            keyframe_guid, keyframe_data = generateKeyframe(img_guid, 'IMAGE', f['duration'])
...
def updateScripts(scripts, project_name):
    for s in scripts:
        if '.meta' in s:
            ...
            updated = re.sub(b'(?<=self\\.getResource\\(\\)\\.getContent\\(")(.*?)(?=")', project_name.encode('utf-8'), scripts[s])
            scripts[s] = updated
...
def updateManifest(manifest, project_name):
    ...
    for c in manifest['content'][0]:
        if c == 'id':
            ...
            manifest['content'][0][c] = project_name + 'was imported by the Fraytools Animation Importer by Zardy Z'
...
def importCharacter():
    ...
    folder_tree['library']['sprites'][session['sprite_name']] = session['sprite_data'][session['sprite_name']]
